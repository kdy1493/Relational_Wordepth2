"""
Relational WorDepth training script.

This is a copy of train.py adapted to:
- use NYURelationalDataset when relational supervision is enabled
- add RelationalDepthLoss on top of the base WorDepth loss

Usage (multi-GPU, DDP example):
...
def _make_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Relational WorDepth PyTorch implementation.",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="base dir for train relations (e.g., data/nyu_relational/median_train)",
    )
    parser.add_argument(
        "--relations_dir_eval",
        type=str,
        default=None,
        help="base dir for eval relations, if any",
    )
    parser.add_argument(
        "--rel_weight",
        type=float,
        default=0.1,
        help="weight for relational loss term added on top of base depth loss",
    )
    parser.add_argument(
        "--rel_margin",
        type=float,
        default=0.1,
        help="margin (m) used in RelationalDepthLoss",
    )
    parser.add_argument(
        "--rel_min_pixels",
        type=int,
...
def main_worker(args: argparse.Namespace) -> None:
    ...
    if is_main:
        ...
        for label, src in [("relational_train.py.backup", "src/relational_train.py"), ("wordepth.py.backup", "src/networks/wordepth.py")]:
    ...
    rel_loss_fn: Optional[RelationalDepthLoss] = None
    if use_relational:
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(data_iter):
            ...
            relations_batch = sample_batched.get("relations") if use_relational else None
...
if args.dataset in ("kitti", "nyu", "nyu_matched"):
    from dataloaders.dataloader import NewDataLoader as _BaseNewDataLoader  # noqa: F401
else:
    _BaseNewDataLoader = None  # type: ignore[misc, assignment]


if __name__ == "__main__":
    if args.mode != "train":
        logger.warning("relational_train.py is for training only. Eval은 src/eval.py 를 사용하세요.")
        sys.exit(0)
